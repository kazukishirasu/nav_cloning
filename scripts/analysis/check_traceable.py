...
class nav_cloning_node:
    def __init__(self):
        ...
        self.threshold = 150
        self.traceable = 'traceable'+str(self.model_num)+'.csv'
        self.trajectory = 'trajectory'+str(self.model_num)+'.csv'
        self.result_path = roslib.packages.get_pkg_dir('nav_cloning')+'/data/result_selected_training/thesis/'+str(self.dirnum)+'/'
        self.checkpoint = roslib.packages.get_pkg_dir('nav_cloning')+'/data/path'+str(self.dirnum)+'.csv'
        print(self.checkpoint)
        self.load_path = roslib.packages.get_pkg_dir('nav_cloning')+'/data/model_selected_training/thesis/'+str(self.dirnum)+'/model'+str(self.model_num)+'.pt'
        ...
        with open(self.checkpoint, 'r') as f:
            for row in csv.reader(f):
                self.checkpoint_list.append(row)
    ...
    def robot_move(self, move_count): #reset
        ...
        state.pose.position.x = float(self.checkpoint_list[move_count][0])
        state.pose.position.y = float(self.checkpoint_list[move_count][1])
        state.pose.orientation.x = 0
        state.pose.orientation.y = 0
        state.pose.orientation.z = float(self.checkpoint_list[move_count][2])
        state.pose.orientation.w = float(self.checkpoint_list[move_count][3])
        try:
    ...
    def loop(self):
        ...
        if self.episode > self.threshold and self.min_distance <= 0.3:
            print("-----------------------------------Success-----------------------------------")
            line = ["Success"]
            ...
            self.episode = 0
            self.move_count += 1
            if self.move_count > len(self.checkpoint_list):
                os.system('killall roslaunch')
                sys.exit()
            self.robot_move(self.move_count-1)

        elif self.episode > self.threshold and self.min_distance > 0.3:
            print("-----------------------------------Failure-----------------------------------")
            line = ["Failure"]
            with open(self.result_path + self.traceable, 'a') as f:
                writer = csv.writer(f, lineterminator='\n')
        ...
        print("move_count:" +str(self.move_count)+'/'+str(len(self.checkpoint_list)))
        self.episode += 1
